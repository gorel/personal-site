...
def resend_verification_email():
    ...
    flask.flash(
        json.dumps({
            "msg": f"Sent verification email to {user.email}",
        }),
        "alert-success",
    )
    return flask.redirect(flask.url_for("default.home"))
...
def verify_account(token):
    ...
    if user is not None:
        ...
        flask.flash(
            json.dumps({
                "msg": "Verified email successfully. Welcome!",
            }),
            "alert-success",
        )
    return flask.redirect(flask.url_for("default.home"))
...
def reset_password(token):
    if flask_login.current_user.is_authenticated:
        flask.flash(
            json.dumps({
                "msg": "You should do that from your profile",
                "link": flask.url_for("profile.edit"),
            }),
            "alert-warning",
        )
